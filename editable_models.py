...
class TransformerEncoderLayer(nn.Module):
    ...
    def _sa_block(self, x: Tensor,
        ...
        x = self.self_attn(x.float(), x.float(), x.float(),
                           attn_mask=attn_mask,
...
class RNN(nn.Module):
    def __init__(self, seq_len,vocab_size,hidden_size):
        super(RNN, self).__init__()

        self.output_size = torch.Size([seq_len,vocab_size])
        self.hidden_size = hidden_size
        self.i2h = nn.Linear(seq_len + hidden_size, hidden_size)
        self.i2o = nn.Linear(seq_len + hidden_size, seq_len)
